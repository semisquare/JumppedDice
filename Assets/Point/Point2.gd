...
func _ready():
	...
	if instigator.get_cell_tile_data(0, instigator.local_to_map(global_position + direction_list[0])):
		print(instigator.get_neighbor_cell(instigator.local_to_map(global_position), TileSet.CELL_NEIGHBOR_TOP_SIDE))
...
func generate_point():
	...
	if direction:
		instigator.set_cell(0, instigator.local_to_map(global_position + direction), 0, Vector2i(0, 0))
